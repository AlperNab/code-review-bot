...
**Domain:** `Developer / Code Quality` · **Suite:** `Developer Productivity Suite` · **Accent:** `#60a5fa`
...
**Code Review Bot** is a standalone, browser-based AI workflow app for **Developer / Code Quality**. It turns structured inputs, uploaded files, and project-specific settings into reviewable outputs using a deterministic local engine plus optional local/cloud LLM enhancement.
...
|---|---:|:---:|---|---|
| `code`<br>Code | text | Yes | — | Affects input: Code. |
| `pr`<br>PR | text | Yes | — | Affects input: PR. |
...
| `style_guide`<br>style guide | select | No | — | Affects customization: style guide. |
| `security_profile`<br>security profile | select | No | — | Affects customization: security profile. |
| `max_comments`<br>max comments | text | No | — | Affects customization: max comments. |
| `ignored_paths`<br>ignored paths | textarea | No | — | Affects customization: ignored paths. |
| `ci_policy`<br>CI policy | text | No | — | Affects customization: CI policy. |
...
uvicorn app.main:app --host 127.0.0.1 --port 9108
```
...
| `AI_SUITE_PORT` | Port for this project GUI, default `9108`. |
...
```bash
curl http://127.0.0.1:9108/api/health
```
...
```bash
curl -X POST http://127.0.0.1:9108/api/run \
...
```bash
AI_SUITE_HOST=0.0.0.0 AI_SUITE_PORT=9108 uvicorn app.main:app --host 0.0.0.0 --port 9108
```
...
- `plugins/code-review-bot.json` for plugin metadata.
